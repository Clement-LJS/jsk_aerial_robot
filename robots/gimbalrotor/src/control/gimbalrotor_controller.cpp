...
void GimbalrotorController::controlCore()
{
  ...
  const bool entering_perching_takeoff_collective =
    perching_takeoff_collective_active &&
    !previous_perching_takeoff_collective_active_;

  if(entering_perching_takeoff_collective)
  {
    start_rp_integration_ = true;

    spinal::FlightConfigCmd integration_cmd;
    integration_cmd.cmd =
      spinal::FlightConfigCmd::INTEGRATION_CONTROL_ON_CMD;

    navigator_->getFlightConfigPublisher().publish(integration_cmd);

    perching_neutral_enter_time_ = ros::Time::now();

    ROS_WARN(
      "[GimbalrotorController] "
      "perching takeoff collective ramp and "
      "roll/pitch integration started");
  }
