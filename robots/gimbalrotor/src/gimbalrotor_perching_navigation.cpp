...
GimbalrotorPerchingNavigator::GimbalrotorPerchingNavigator():
  ...
  perching_pitch_joint_enable_(true),
  pitch_joint_target_initialized_(false),
  pitch_joint_measurement_valid_(false),
  pitch_joint_prev_measurement_valid_(false),
  pitch_joint_prev_command_initialized_(false),
  pitch_joint_ready_(false),
  pitch_joint_drift_fault_(false),
  ...
  pitch_joint_measured_position_(0.0),
  pitch_joint_prev_measured_position_(0.0),
  pitch_joint_measured_velocity_(0.0),
  pitch_joint_requested_target_(0.0),
  pitch_joint_last_published_command_(0.0),
  pitch_joint_locked_angle_(std::numeric_limits<double>::quiet_NaN()),
  pitch_joint_min_(-1.57),
  pitch_joint_max_(1.57),
  pitch_joint_max_velocity_(0.1),
  pitch_joint_command_pub_interval_(0.02),
  pitch_joint_position_tolerance_(0.03),
  pitch_joint_velocity_tolerance_(0.02),
  pitch_joint_stable_duration_(0.3),
  pitch_joint_drift_tolerance_(0.05),

  pivot_source_("manual"),
  perching_takeoff_target_pitch_frame_("world"),
  pitch_joint_name_("pitch_joint"),
  perching_center_link_name_("perching_center"),
  ...
  pitch_joint_command_topic_("joints_ctrl"),
  pitch_joint_target_topic_("perching/pitch_joint/target"),
  pitch_joint_ready_topic_("perching/pitch_joint/ready"),
  pitch_joint_locked_angle_topic_("perching/pitch_joint/locked_angle"),
  ...
  locked_x_side_(1.0),
  pitch_joint_ready_start_time_(0),
  pitch_joint_prev_measurement_time_(0),
  pitch_joint_last_command_pub_time_(0),
    ...
    double loop_du)
{
  ...
  manual_pitch_delta_sub_ = nh_.subscribe(
      ...
      this);
  pitch_joint_target_sub_ = nh_.subscribe(
      pitch_joint_target_topic_,
      1,
      &GimbalrotorPerchingNavigator::pitchJointTargetCallback,
      this);
  ...
  pitch_joint_command_pub_ = nh_.advertise<sensor_msgs::JointState>(pitch_joint_command_topic_, 1);
  pitch_joint_ready_pub_ = nh_.advertise<std_msgs::Bool>(pitch_joint_ready_topic_, 1, true);
  pitch_joint_locked_angle_pub_ = nh_.advertise<std_msgs::Float64>(pitch_joint_locked_angle_topic_, 1, true);
  perching_constraint_valid_pub_ = nh_.advertise<std_msgs::Bool>(perching_constraint_valid_topic_, 1, true);

  std_msgs::Bool ready_msg;
  ready_msg.data = false;
  pitch_joint_ready_pub_.publish(ready_msg);
  perching_constraint_valid_pub_.publish(ready_msg);

  std_msgs::Float64 locked_angle_msg;
  locked_angle_msg.data = std::numeric_limits<double>::quiet_NaN();
  pitch_joint_locked_angle_pub_.publish(locked_angle_msg);
  ...
  ROS_WARN("[GimbalrotorPerchingNavigator] pitch joint target topic: %s", pitch_joint_target_topic_.c_str());
...
void GimbalrotorPerchingNavigator::rosParamInit()
{
  ...
  getParam<bool>(navi_nh, "perching_pitch_joint_enable", perching_pitch_joint_enable_, true);
  getParam<std::string>(navi_nh, "perching_pitch_joint_name", pitch_joint_name_, std::string("pitch_joint"));
  getParam<std::string>(navi_nh, "perching_center_link_name", perching_center_link_name_, std::string("perching_center"));
  getParam<std::string>(navi_nh, "perching_pitch_joint_command_topic", pitch_joint_command_topic_, std::string("joints_ctrl"));
  getParam<std::string>(navi_nh, "perching_pitch_joint_target_topic", pitch_joint_target_topic_, std::string("perching/pitch_joint/target"));
  getParam<std::string>(navi_nh, "perching_pitch_joint_ready_topic", pitch_joint_ready_topic_, std::string("perching/pitch_joint/ready"));
  getParam<std::string>(navi_nh, "perching_pitch_joint_locked_angle_topic", pitch_joint_locked_angle_topic_, std::string("perching/pitch_joint/locked_angle"));
  getParam<std::string>(navi_nh, "perching_constraint_valid_topic", perching_constraint_valid_topic_, std::string("perching/constraint_valid"));
  getParam<double>(navi_nh, "perching_pitch_joint_min", pitch_joint_min_, -1.57);
  getParam<double>(navi_nh, "perching_pitch_joint_max", pitch_joint_max_, 1.57);
  getParam<double>(navi_nh, "perching_pitch_joint_max_velocity", pitch_joint_max_velocity_, 0.1);
  getParam<double>(navi_nh, "perching_pitch_joint_command_pub_interval", pitch_joint_command_pub_interval_, 0.02);
  getParam<double>(navi_nh, "perching_pitch_joint_position_tolerance", pitch_joint_position_tolerance_, 0.03);
  getParam<double>(navi_nh, "perching_pitch_joint_velocity_tolerance", pitch_joint_velocity_tolerance_, 0.02);
  getParam<double>(navi_nh, "perching_pitch_joint_stable_duration", pitch_joint_stable_duration_, 0.30);
  getParam<double>(navi_nh, "perching_pitch_joint_drift_tolerance", pitch_joint_drift_tolerance_, 0.05);
  ...
  if(!std::isfinite(pitch_joint_min_)) pitch_joint_min_ = -1.57;
  if(!std::isfinite(pitch_joint_max_)) pitch_joint_max_ = 1.57;
  if(pitch_joint_min_ > pitch_joint_max_) std::swap(pitch_joint_min_, pitch_joint_max_);
  if(!std::isfinite(pitch_joint_max_velocity_) || pitch_joint_max_velocity_ < 0.0) pitch_joint_max_velocity_ = 0.1;
  if(!std::isfinite(pitch_joint_command_pub_interval_) || pitch_joint_command_pub_interval_ <= 0.0) pitch_joint_command_pub_interval_ = 0.02;
  if(!std::isfinite(pitch_joint_position_tolerance_) || pitch_joint_position_tolerance_ < 0.0) pitch_joint_position_tolerance_ = 0.03;
  if(!std::isfinite(pitch_joint_velocity_tolerance_) || pitch_joint_velocity_tolerance_ < 0.0) pitch_joint_velocity_tolerance_ = 0.02;
  if(!std::isfinite(pitch_joint_stable_duration_) || pitch_joint_stable_duration_ < 0.0) pitch_joint_stable_duration_ = 0.30;
  if(!std::isfinite(pitch_joint_drift_tolerance_) || pitch_joint_drift_tolerance_ < 0.0) pitch_joint_drift_tolerance_ = 0.05;
}

void GimbalrotorPerchingNavigator::update()
{
  updatePitchJointState();
...
void GimbalrotorPerchingNavigator::perchingEnableCallback(const std_msgs::BoolConstPtr& msg)
{
  ...
  else
  {
    ...
    bool_msg.data = false;
    perching_constraint_valid_pub_.publish(bool_msg);
    std_msgs::Float64 float_msg;
    float_msg.data = pitch_joint_locked_angle_;
    pitch_joint_locked_angle_pub_.publish(float_msg);
  }
}
...
void GimbalrotorPerchingNavigator::relockCallback(const std_msgs::EmptyConstPtr& msg)
{
  ...
  if(perching_pitch_joint_enable_ &&
     (!pitch_joint_measurement_valid_ || !pitch_joint_ready_))
  {
    ROS_ERROR(
        "[GimbalrotorPerchingNavigator] "
        "perching relock rejected because pitch joint is not ready");
    return;
  }
...
void GimbalrotorPerchingNavigator::manualPitchDeltaCallback(const std_msgs::Float64ConstPtr& msg)
{
  ...
  if(perching_pitch_joint_enable_ && !perching_constraint_valid_)
  {
    ROS_ERROR_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] manual pitch delta ignored because the perching constraint is invalid");
    return;
  }
...
{
  if(perching_locked_ && perching_lock_once_ && perching_constraint_valid_)
  {
    return true;
  }
  ...
  if(perching_pitch_joint_enable_)
  {
    if(pitch_joint_drift_fault_)
    {
      ROS_ERROR_THROTTLE(
          1.0,
          "[GimbalrotorPerchingNavigator] cannot lock: pitch joint drift fault is latched");
      return false;
    }

    if(!pitch_joint_measurement_valid_ || !pitch_joint_ready_ ||
       !std::isfinite(pitch_joint_requested_target_) ||
       !std::isfinite(pitch_joint_measured_position_))
    {
      ROS_WARN_THROTTLE(
          1.0,
          "[GimbalrotorPerchingNavigator] cannot lock: pitch joint is not ready");
      return false;
    }
  }
  ...
  if(!std::isfinite(locked_pivot_world_.x()) ||
     !std::isfinite(locked_pivot_world_.y()) ||
     !std::isfinite(locked_pivot_world_.z()))
  {
    ROS_ERROR_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] cannot lock: pivot computation failed");
    return false;
  }
  ...
  perching_constraint_valid_ = true;
  pitch_joint_locked_angle_ = perching_pitch_joint_enable_
      ? pitch_joint_measured_position_
  ...
  publishLockedDebugPose();
  publishLockedPivot();
  std_msgs::Bool constraint_msg;
  constraint_msg.data = true;
  perching_constraint_valid_pub_.publish(constraint_msg);

  std_msgs::Float64 locked_angle_msg;
  locked_angle_msg.data = pitch_joint_locked_angle_;
  pitch_joint_locked_angle_pub_.publish(locked_angle_msg);
...
void GimbalrotorPerchingNavigator::naviCallback(const aerial_robot_msgs::FlightNavConstPtr& msg)
{
  ...
  if(perching_enable_ && !applyPerchingConstraint(nav_msg))
  {
    ROS_ERROR_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] rejected navigation command because the perching constraint is invalid");
    return;
  }
...
void GimbalrotorPerchingNavigator::pitchJointTargetCallback(const std_msgs::Float64ConstPtr& msg)
{
  if(!std::isfinite(msg->data))
  {
    ROS_WARN_THROTTLE(1.0, "[GimbalrotorPerchingNavigator] rejected non-finite pitch-joint target");
    return;
  }

  if(perching_locked_ || perching_constraint_valid_ ||
     perching_takeoff_transition_active_ || perching_landing_transition_active_ ||
     perching_takeoff_fault_)
  {
    ROS_WARN_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] rejected pitch-joint target while perching geometry is locked or transitioning");
    return;
  }

  pitch_joint_requested_target_ = clamp(msg->data, pitch_joint_min_, pitch_joint_max_);
  pitch_joint_target_initialized_ = true;
  pitch_joint_ready_ = false;
  pitch_joint_ready_start_time_ = ros::Time(0);
}

void GimbalrotorPerchingNavigator::updatePitchJointState()
{
  if(!perching_pitch_joint_enable_)
  {
    return;
  ...
  const bool valid_measurement = readCurrentPitchJointPosition(measured_position);

  pitch_joint_measurement_valid_ = valid_measurement;
  ...
  {
    pitch_joint_measured_position_ = measured_position;

    if(!pitch_joint_target_initialized_)
    {
      pitch_joint_requested_target_ = measured_position;
      pitch_joint_last_published_command_ = measured_position;
      pitch_joint_target_initialized_ = true;
      pitch_joint_prev_command_initialized_ = true;
    }

    if(pitch_joint_prev_measurement_valid_)
    {
      const double dt = (now - pitch_joint_prev_measurement_time_).toSec();
      if(std::isfinite(dt) && dt > 1.0e-4 && dt < 0.5)
      {
        pitch_joint_measured_velocity_ =
            (measured_position - pitch_joint_prev_measured_position_) / dt;
      }
      else
      {
        pitch_joint_measured_velocity_ = 0.0;
    ...
    pitch_joint_prev_measured_position_ = measured_position;
    pitch_joint_prev_measurement_time_ = now;
    pitch_joint_prev_measurement_valid_ = true;

    updatePitchJointReadyState(now);
  }

  publishPitchJointCommandIfNeeded();

  if(perching_constraint_valid_ &&
     std::isfinite(pitch_joint_locked_angle_) &&
     valid_measurement &&
     std::fabs(measured_position - pitch_joint_locked_angle_) > pitch_joint_drift_tolerance_)
  {
    pitch_joint_drift_fault_ = true;

    if(perching_takeoff_transition_active_)
    {
      perching_constraint_valid_ = false;
      latchPerchingTakeoffFault("pitch joint moved after geometry lock");
      return;
    }

    invalidatePerchingConstraint("pitch joint moved after geometry lock");
  }
}

void GimbalrotorPerchingNavigator::publishPitchJointCommandIfNeeded()
{
  if(!perching_pitch_joint_enable_ || !pitch_joint_target_initialized_)
  {
    return;
  }

  const ros::Time now = ros::Time::now();
  const double since_last_pub = (now - pitch_joint_last_command_pub_time_).toSec();

  if(!pitch_joint_last_command_pub_time_.isZero() &&
     since_last_pub < pitch_joint_command_pub_interval_)
  {
    return;
  }

  double target = pitch_joint_requested_target_;
  if(perching_constraint_valid_ && std::isfinite(pitch_joint_locked_angle_))
  {
    target = pitch_joint_locked_angle_;
  }

  if(!pitch_joint_prev_command_initialized_)
  {
    pitch_joint_last_published_command_ =
        pitch_joint_measurement_valid_ ? pitch_joint_measured_position_ : target;
    pitch_joint_prev_command_initialized_ = true;
  }
  ...
  if(!pitch_joint_last_command_pub_time_.isZero())
  {
    const double dt = (now - pitch_joint_last_command_pub_time_).toSec();
    if(std::isfinite(dt) && dt > 0.0)
    {
      const double max_step = pitch_joint_max_velocity_ * dt;
      const double error = target - pitch_joint_last_published_command_;
      command = pitch_joint_last_published_command_ + clamp(error, -max_step, max_step);
    }
  }

  command = clamp(command, pitch_joint_min_, pitch_joint_max_);

  sensor_msgs::JointState joint_msg;
  joint_msg.header.stamp = now;
  joint_msg.name.push_back(pitch_joint_name_);
  joint_msg.position.push_back(command);
  pitch_joint_command_pub_.publish(joint_msg);

  pitch_joint_last_published_command_ = command;
  pitch_joint_last_command_pub_time_ = now;
}

void GimbalrotorPerchingNavigator::updatePitchJointReadyState(const ros::Time& now)
{
  const bool stable =
      pitch_joint_measurement_valid_ &&
      std::isfinite(pitch_joint_measured_position_) &&
      std::isfinite(pitch_joint_requested_target_) &&
      std::isfinite(pitch_joint_measured_velocity_) &&
      std::fabs(pitch_joint_measured_position_ - pitch_joint_requested_target_) <= pitch_joint_position_tolerance_ &&
      std::fabs(pitch_joint_measured_velocity_) <= pitch_joint_velocity_tolerance_;
  ...
  else if(pitch_joint_ready_start_time_.isZero())
  {
    pitch_joint_ready_start_time_ = now;
    pitch_joint_ready_ = pitch_joint_stable_duration_ <= 0.0;
  }
  else
  {
    pitch_joint_ready_ = (now - pitch_joint_ready_start_time_).toSec() >= pitch_joint_stable_duration_;
  }

  std_msgs::Bool ready_msg;
  ready_msg.data = pitch_joint_ready_;
  pitch_joint_ready_pub_.publish(ready_msg);
}

void GimbalrotorPerchingNavigator::invalidatePerchingConstraint(const std::string& reason)
{
  perching_constraint_valid_ = false;
  perching_locked_ = false;
  has_active_pitch_target_ = false;
  ...
  std_msgs::Bool constraint_msg;
  constraint_msg.data = false;
  perching_constraint_valid_pub_.publish(constraint_msg);

  ROS_ERROR_THROTTLE(1.0, "[GimbalrotorPerchingNavigator] %s", reason.c_str());
}
...
tf::Vector3 GimbalrotorPerchingNavigator::computeHandPerchingCenterWorldFromBaselink() const
{
  if(perching_pitch_joint_enable_)
  {
    tf::Vector3 perching_center_world;
    if(computePerchingCenterWorldFromFk(perching_center_world))
    {
      return perching_center_world;
    }

    ROS_ERROR_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] failed to compute FK perching center; refusing to use the old fixed offset while pitch-joint mode is enabled");
    return tf::Vector3(std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN(),
                       std::numeric_limits<double>::quiet_NaN());
  }
...
bool GimbalrotorPerchingNavigator::readCurrentPitchJointPosition(double& position) const
{
  const KDL::JntArray& joint_positions =
      robot_model_->getJointPositions();

  const unsigned int expected_joint_count =
      robot_model_->getTree().getNrOfJoints();

  /*
   * The robot model does not contain valid joint positions until the first
   * /joint_states message has been received.
   *
   * The navigation update timer can run before that message arrives.
   * Therefore, verify the KDL joint-array size before accessing it.
   */
  if(joint_positions.rows() != expected_joint_count)
  {
    ROS_WARN_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] waiting for a complete joint state "
        "(received %u model joints, expected %u)",
        static_cast<unsigned int>(joint_positions.rows()),
  ...
  const sensor_msgs::JointState joint_state =
      robot_model_->kdlJointToMsg(joint_positions);

  if(joint_state.name.size() != joint_state.position.size())
  {
    ROS_WARN_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] pitch joint state is invalid because "
        "name and position sizes differ");
  ...
  ROS_WARN_THROTTLE(
      1.0,
      "[GimbalrotorPerchingNavigator] pitch joint [%s] not found in robot model",
      pitch_joint_name_.c_str());

  return false;
}

bool GimbalrotorPerchingNavigator::computePerchingCenterWorldFromFk(
    tf::Vector3& perching_center_world) const
{
  ...
  /*
   * Do not run forward kinematics before the robot model has received its
   * first complete joint-state update.
   */
  if(joint_positions.rows() != tree.getNrOfJoints())
  {
    ROS_WARN_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] cannot compute perching-center FK "
        "before a complete joint state is available "
        "(received %u model joints, expected %u)",
        static_cast<unsigned int>(joint_positions.rows()),
        tree.getNrOfJoints());
  ...
  KDL::Frame root_to_perching_center;
  ...
  const int perching_center_fk_result =
      fk_solver.JntToCart(
          joint_positions,
          root_to_perching_center,
          perching_center_link_name_);

  if(base_fk_result < 0 || perching_center_fk_result < 0)
  {
    ROS_ERROR_THROTTLE(
        1.0,
        "[GimbalrotorPerchingNavigator] FK failed for base_link or %s",
        perching_center_link_name_.c_str());

    return false;
  }

  const KDL::Frame base_to_perching_center =
      root_to_base.Inverse() * root_to_perching_center;

  tf::Vector3 base_to_perching_center_tf;

  tf::vectorKDLToTF(
      base_to_perching_center.p,
      base_to_perching_center_tf);

  if(!std::isfinite(base_to_perching_center_tf.x()) ||
     !std::isfinite(base_to_perching_center_tf.y()) ||
     !std::isfinite(base_to_perching_center_tf.z()))
  {
    return false;
  }

  perching_center_world =
      getCurrentBaselinkPos() +
      getCurrentBaselinkRot() * base_to_perching_center_tf;

  return std::isfinite(perching_center_world.x()) &&
         std::isfinite(perching_center_world.y()) &&
         std::isfinite(perching_center_world.z());
}
...
bool GimbalrotorPerchingNavigator::applyPerchingConstraint(aerial_robot_msgs::FlightNav& nav_msg)
{
  if(!perching_enable_)
  {
    return true;
  ...
  if(perching_pitch_joint_enable_ && !perching_constraint_valid_ && perching_locked_)
  {
    return false;
  }
  ...
  return perching_constraint_valid_;
}
